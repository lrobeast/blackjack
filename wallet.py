...
STARTING_BALANCE = (
    10000  # -> Variable qui definit le solde de départ pour chaque joueur
)
...
def load_wallets():  # ->fonction permettant de reprendre la ou on en etait en chargant les donnee du wallet depuis le fichier json ou d en creer un nouveau
    ...
    with open(WALLET_FILE, "r") as f:  # ->on ouvre le wallet en mode lecture
        return json.load(
            f
        )  # ->on charge les données du wallet depuis le fichier json et on les retourne sous forme de dictionnaire


def save_wallets(wallets):  # ->fonction qui permet de sauvegarder ses données
    os.makedirs(
        "data", exist_ok=True
    )  # ->on cree le dossier si il n existe pas deja grace a exist_ok=True
    with open(
        WALLET_FILE, "w"
    ) as f:  # ->ou on ouvre le wallet en mode ecriture pour modifer les anciennes donnes et mettre les nouvelles
        json.dump(
            wallets, f, indent=2
        )  # ->on met toutes les données dans le fichier json
...
def update_balance(
    username, amount
):  # ->sert a changer la solde du joueur en cas de gains ou de pertes
